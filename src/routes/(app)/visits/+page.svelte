<script lang="ts">
	...
	const terms = $derived(recordTerms(page.data.terms, 'visit'));
...
<div class="space-y-6">
	<PageHeader.Root>
		...
		{#if data.canCreate}
			<PageHeader.Actions>
				<CreateRecord type="visit" form={data.createForm} pickers={data.createPickers} />
			</PageHeader.Actions>
		{/if}
	</PageHeader.Root>
...
<DeleteRecord type="visit" form={data.deleteForm} query={QUERY.visits} bind:removing />
