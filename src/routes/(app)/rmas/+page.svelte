<script lang="ts">
	...
	const terms = $derived(recordTerms(page.data.terms, 'rma'));
...
<div class="space-y-6">
	<PageHeader.Root>
		...
		{#if data.canCreate}
			<PageHeader.Actions>
				<CreateRecord type="rma" form={data.createForm} />
			</PageHeader.Actions>
		{/if}
	</PageHeader.Root>
...
<DeleteRecord type="rma" form={data.deleteForm} query={QUERY.rmas} bind:removing />
