<script lang="ts">
	...
	const terms = $derived(recordTerms(page.data.terms, 'coupon'));
...
<div class="space-y-6">
	<PageHeader.Root>
		...
		{#if data.canCreate}
			<PageHeader.Actions>
				<CreateRecord type="coupon" form={data.createForm} />
			</PageHeader.Actions>
		{/if}
	</PageHeader.Root>
...
<DeleteRecord type="coupon" form={data.deleteForm} query={QUERY.coupons} bind:removing />
