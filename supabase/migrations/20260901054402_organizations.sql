...
-- billing code can add or rename plans without a schema migration.
...
-- their full name, falling back to the local part of their email.
...
begin
	for u in
		select
			users.id,
			coalesce(users.raw_user_meta_data ->> 'full_name', split_part(users.email, '@', 1)) as org_name
